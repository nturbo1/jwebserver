...
public class HttpMsgParserTest {
    ...
    void returnCorrectHttpMethod(String method) throws HttpMessageParseException, IOException {
        // A space character is added at the end of the input string, `method`, to simulate a valid end of the
        // HTTP Method section in an HTTP request line as if the bytes are coming from a network input stream.
        InputStream iStream = new ByteArrayInputStream((method + " ").getBytes(StandardCharsets.UTF_8));
        HttpMethod parsedMethod = HttpMsgParser.parseHttpMethod(iStream);
    ...
    void returnValidHttpVersion(String versionTxt, double expectedVersion) throws HttpMessageParseException, IOException {
        // A space character is added at the end of the input string, `versionTxt`, to simulate a valid end of the
        // HTTP version section in an HTTP request line as if the bytes are coming from a network input stream.
        InputStream iStream = new ByteArrayInputStream((versionTxt + " ").getBytes(StandardCharsets.UTF_8));
        double parsedVersion = HttpMsgParser.parseHttpVersion(iStream);
    ...
    {
        InputStream iStream = new ByteArrayInputStream(invalidVersionTxt.getBytes(StandardCharsets.UTF_8));
        Assertions
                .assertThatThrownBy(() -> HttpMsgParser.parseHttpVersion(iStream))
                .isInstanceOf(HttpMessageParseException.class);
    }

    @Test
    void throwHttpMessageParseExceptionForUnknownMethod()
    {
        InputStream iStream = new ByteArrayInputStream("asdfasdf".getBytes(StandardCharsets.UTF_8));
        Assertions
                .assertThatThrownBy(() -> HttpMsgParser.parseHttpVersion(iStream))
                .isInstanceOf(HttpMessageParseException.class);
    }
...
//            throws InvalidHttpHeaderException, IOException
//    {
//        InputStream is = new ByteArrayInputStream(testHeaders.headersBytes);
//        Map<String, List<String>> parsedHeadersMap = HttpMsgParser.parseHttpHeadersSection(is);
//
...
//        Assertions.assertThatThrownBy(() -> HttpParser.parseHttpHeadersSection(is));
//    }
